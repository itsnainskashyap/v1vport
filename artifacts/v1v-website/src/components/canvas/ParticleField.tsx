...
export function ParticleField({ count, scrollProgress }: Props) {
  ...
  const mainData = useMemo(() => {
    ...
    for (let i = 0; i < count; i++) {
      const i3 = i * 3;
      const radius = 1.5 + Math.random() * 25;
      const theta = Math.random() * Math.PI * 2;
      const phi = Math.acos(2 * Math.random() - 1);
      const zSpread = (Math.random() - 0.5) * 130;
  ...
  const nebulaCount = Math.floor(count * 0.3);
  const nebulaData = useMemo(() => {
    ...
    for (let i = 0; i < nebulaCount; i++) {
      ...
      colors[i3 + 1] = brightness * (0.6 + Math.random() * 0.4);
  ...
  useFrame((state) => {
    ...
    if (pointsRef.current) {
      ...
      for (let i = 0; i < count; i++) {
        const i3 = i * 3;
        const speed = 0.03 + (i % 13) * 0.003;
        arr[i3] = mainData.basePositions[i3] + Math.sin(t * speed + i * 0.31) * 0.15;
        arr[i3 + 1] = mainData.basePositions[i3 + 1] + Math.cos(t * speed * 0.6 + i * 0.19) * 0.15;
        arr[i3 + 2] = mainData.basePositions[i3 + 2] + Math.sin(t * speed * 0.3 + i * 0.43) * 0.1;
      }
    ...
    if (nebulaRef.current) {
      ...
      for (let i = 0; i < nebulaCount; i++) {
        const i3 = i * 3;
        arr[i3] = nebulaData.basePositions[i3] + Math.sin(t * 0.1 + i * 0.4) * 0.6;
        arr[i3 + 1] = nebulaData.basePositions[i3 + 1] + Math.cos(t * 0.08 + i * 0.25) * 0.4;
        arr[i3 + 2] = nebulaData.basePositions[i3 + 2] + Math.sin(t * 0.06 + i * 0.6) * 0.5;
      }
      pos.needsUpdate = true;
    }
  });
