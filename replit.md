...
- `components/SoundManager.tsx` — Procedural ambient sound using Web Audio API. 3 sine pad oscillators (65Hz, 98Hz, 131Hz), bandpass-filtered noise atmosphere, LFO modulation. Volume reacts to scroll position (asteroid belt bass boost, contact fadeout). Scroll-reactive audio layers: deep bass rumble near asteroid belt (35Hz+55Hz), ethereal high tones in open space (2200Hz+3300Hz), whoosh noise on fast scroll (bandpass filtered white noise with rAF-driven decay). Mute/unmute button.
...
- `components/canvas/ScrollScene.tsx` — Camera travels z=8 → z=-80 with cinematic serpentine (sin×1.5 X, sin×0.8 Y). Camera shake on fast scroll and near text sections. 11 text sections defined for camera auto-alignment. Texts appear/disappear sequentially. Integrates all visual components: SpeedLines, NebulaClouds, AsteroidBelt, PlanetFlyby, DynamicLighting, LensFlares.
- `components/canvas/SpeedLines.tsx` — 600 radial streak particles that appear when scrolling fast. Particles stream from behind toward camera, creating a hyperspace/warp effect. Intensity proportional to scroll speed.
- `components/canvas/NebulaClouds.tsx` — 5 large nebula cloud planes at key z-positions using canvas-generated gradient textures. Additive blending, distance-based fade, subtle rotation/drift animation.
- `components/canvas/EngineFlame.tsx` — 120-particle engine flame/thruster effect attached to spaceship rear. Intensifies with scroll speed, dims when idle. Orange-white gradient with additive blending.
- `components/canvas/AsteroidBelt.tsx` — 60 procedurally-generated asteroid meshes (deformed icosahedra) centered at z=-45, spread across 12-unit depth. Slow rotation and gentle drift. Extra point light at center.
- `components/canvas/PlanetFlyby.tsx` — Large detailed planet at z=-48, x=12 with procedurally-generated surface texture (bands + craters). Double atmosphere glow (inner + outer backside shell). Distance-based visibility.
- `components/canvas/DynamicLighting.tsx` — 5-zone color gradient lighting that shifts as user scrolls: cool blue → deep purple → warm pink → teal-green → amber-orange. Pre-computed THREE.Color objects for zero GC allocation per frame.
- `components/canvas/LensFlares.tsx` — 4 star flare sprites at key positions with canvas-generated cross-hatch textures. Distance-based fade, pulsing intensity animation. Additive blending.
