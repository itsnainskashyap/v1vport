...
- `components/canvas/FloatingImages.tsx` — 21 AI-generated decorative PNG images stationary in the scene (no bobbing). Slow rotation only. Camera flies past them as user scrolls. Additive blending with distance-based fade. Spread from z=-3 to z=-77.
...
- `components/UIOverlay.tsx` — Scroll indicator, "DNA OF CREATIVITY" label, "SELECTED WORK" project list. Contact panel: frosted glass card with "LET'S WORK TOGETHER" heading, form fields, WhatsApp button (green, links to wa.me), Instagram/LinkedIn/Twitter/GitHub icon buttons. Social links loaded from API settings (admin-editable). Gradient scroll progress bar.
